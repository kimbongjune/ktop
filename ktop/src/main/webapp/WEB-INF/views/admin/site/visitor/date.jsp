...
<form name="search_form" method="get" action="<c:url value='/admin/site/visitor/date' />">
...
<ul class="visit_summary_wrap two"> 
	<li> 
		<div class="txt01">최다 방문 일</div>
		<div class="txt02">
			<c:choose>
				<c:when test="${not empty dailyStats}">
					<c:set var="maxStat" value="${dailyStats[0]}"/>
					${maxStat.visitDate}<span class="txt03"><fmt:formatNumber value="${maxStat.percentage}" pattern="0.00"/>%</span>
				</c:when>
				<c:otherwise>
	...
	<li>
		<div class="txt01">최저 방문 일</div>
		<div class="txt02">
			<c:choose>
				<c:when test="${not empty dailyStats}">
					<c:set var="minStat" value="${dailyStats[dailyStats.size()-1]}"/>
					${minStat.visitDate}<span class="txt03"><fmt:formatNumber value="${minStat.percentage}" pattern="0.00"/>%</span>
				</c:when>
				<c:otherwise>
					-<span class="txt03">0%</span>
				</c:otherwise>
	...
	<c:choose>
		...
		<c:otherwise>
			<c:forEach var="stat" items="${dailyStats}" varStatus="status">
				<tr>
					<td class="center"><a href="<c:url value='/admin/site/visitor?startDate=${stat.visitDate}&endDate=${stat.visitDate}' />">${stat.visitDate}</a></td>
