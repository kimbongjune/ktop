...
<form name="admin_popup_form" method="post" action="<c:url value='/admin/site/popup/edit/${popup.id}' />">
...
<tr>
    ...
    <td colspan="3"><input type="text" id="title" name="title" class="input_form w90 required" title="제목" value="${popup.title}" /></td>
</tr>
<tr>
    <th>기간 <em class="fpilsu">*</em></th>
    <td>
		<input type="text" id="startDate" name="startDate" class="input_form input_datepicker w90p" title="시작일" value="${popup.startAt.substring(0, 10)}" />
		<span>부터&nbsp;&nbsp;</span>
		<input type="text" id="endDate" name="endDate" class="input_form input_datepicker w90p" title="종료일" value="${popup.endAt.substring(0, 10)}" />
		<span>까지</span>
    </td>
	<th><label for="isVisible">출력여부</label></th>
    <td>
		<select id="isVisible" name="isVisible">
			<option value="1" ${popup.isVisible == 1 ? 'selected' : ''}>예</option>
			<option value="0" ${popup.isVisible == 0 ? 'selected' : ''}>아니오</option>
		</select>
	</td>
</tr>
<tr>
    <td colspan="4">
		<textarea id="contentHtml" name="contentHtml" class="textarea_form h200p required summernote" title="내용" data-table="guava_popup" data-target="${popup.id}">${popup.contentHtml}</textarea>
	</td>
</tr>
</tbody>
</table>
