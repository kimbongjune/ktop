...
<form name="search_form" method="get" action="<c:url value='/admin/site/visitor/time' />">
...
<ul class="visit_summary_wrap two"> 
		...
		<div class="txt02">
			<c:choose>
				<c:when test="${not empty hourlyStats}">
					<c:set var="maxStat" value="${hourlyStats[0]}"/>
					${maxStat.visitHour < 10 ? '0' : ''}${maxStat.visitHour}:00~${maxStat.visitHour < 10 ? '0' : ''}${maxStat.visitHour}:59<span class="txt03">${maxStat.visitCount}명</span>
				</c:when>
				<c:otherwise>
					-<span class="txt03">0명</span>
				</c:otherwise>
			</c:choose>
		</div>
	</li>
	<li>
		...
		<div class="txt02">
			<c:choose>
				<c:when test="${not empty hourlyStats}">
					<c:set var="minStat" value="${hourlyStats[hourlyStats.size()-1]}"/>
					${minStat.visitHour < 10 ? '0' : ''}${minStat.visitHour}:00~${minStat.visitHour < 10 ? '0' : ''}${minStat.visitHour}:59<span class="txt03">${minStat.visitCount}명</span>
				</c:when>
				<c:otherwise>
					-<span class="txt03">0명</span>
				</c:otherwise>
...
<tbody>
	<c:choose>
		<c:when test="${empty hourlyStats}">
			<tr>
				<td colspan="4" class="center">데이터가 없습니다.</td>
			</tr>
		</c:when>
		<c:otherwise>
			<c:forEach var="stat" items="${hourlyStats}" varStatus="status">
				<tr>
					<td class="center">${stat.visitHour < 10 ? '0' : ''}${stat.visitHour}</td>
...
<tr>
	...
	<td class="right">
		...
		<c:forEach var="stat" items="${hourlyStats}">
