...
function summernote_upFile(file, el) {
	...
	if (!file.type.startsWith('image/')) {
		alert('허용되지 않는 파일 형식입니다. 이미지 파일만 업로드할 수 있습니다.');
		return;
	}
	...
	console.log(file, $(el).attr("name"))
	const form_data = new FormData();
	form_data.append('file', file);

	axios.post("/ktop/file/upload/editor", form_data, {
		headers: {
			'Content-Type': 'multipart/form-data'
		}
	})
	.then(function(response) {
		const img_name = response.data;
...
function summernote_delFile(file) {
	console.log(file)
	axios.delete('/ktop/file/upload/editor', {
	    params: {
	        filePath: file  // 에디터에 삽입된 이미지 경로
	    }
	}).then(res => {
	    console.log(res)
		if (res.data === 'success') {
...
$(function() {
	$('.summernote').summernote({
		height: ((((document.documentElement.clientWidth) ? document.documentElement.clientWidth : window.innerWidth) > 1024) ? 500 : 300),
		minHeight: null,
		maxHeight: null,
		tabsize: 2,
		focus:false,
		lang: 'ko-KR',
		placeholder : '',
		toolbar : [ ['font', ['bold', 'italic', 'underline', 'strikethrough', 'clear']],['fontsize', ['fontsize', 'height', 'color']],['para', ['paragraph']],['insert', ['picture', 'link', 'table']],['view', ['codeview']] ],
		callbacks: {
